...
describe("RedisService", () => {
  ...
  beforeEach(() => {
    ...
    spyOn(mockClient, 'connect').mockImplementation(() => Promise.resolve() as unknown as Promise<RedisClientType>);
    spyOn(mockClient, 'set').mockImplementation((): Promise<any> => Promise.resolve('OK'));
    spyOn(mockClient, 'get').mockImplementation((): Promise<any> => Promise.resolve('value'));
    spyOn(mockClient, 'keys').mockImplementation((): Promise<any> => Promise.resolve(['key1', 'key2']));
    spyOn(mockClient, 'del').mockImplementation((): Promise<any> => Promise.resolve(1));
